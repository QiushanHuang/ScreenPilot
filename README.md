...
ScreenPilot brings those everyday actions together. Choose **blackout** when you want to keep the desktop layout, or **stop Mac output** when you want to remove an unused desktop and let a compatible monitor enter its no-signal standby state. For standby, enable the monitor’s no-signal sleep setting. A display still receiving a signal from another computer will continue showing that input.
...
- **Coordinates the desk as a whole.** Brightness, active Mac outputs and a configured Windows companion live in one workflow. The companion can request input changes from the Windows side, useful when a monitor no longer accepts commands from its inactive Mac input. Enable DDC/CI on the monitor and test both switching directions during setup.
- **Makes “dark” and “disconnected” separate choices.** Software blackout preserves the desktop; stopping output can let an unused LCD sleep. Choose the mode that suits your next task: keep windows in place for a short break, or free up the desktop when a screen is no longer needed.
- **Makes recurring setups quick to use and recover.** Named presets, a built-in-screen shortcut, timed restoration and an independent recovery helper reduce repeated setup work. At least one Mac screen stays active, and a failed restoration leaves a retry option available.
...
**For a shared Mac/Windows desk:** connect both computers to the monitor, use a physical USB switch for your keyboard and mouse, and keep both computers on the LAN. Follow [hardware setup](docs/hardware-setup.md) to register your tested monitor, build the configured Mac app and pair Windows.
...
The Windows executable is **not Authenticode signed**. If your OS blocks launch, review the download source and checksum first; on macOS use **System Settings → Privacy & Security → Open Anyway** if offered. On a managed computer, contact your administrator if launch is blocked.
...
The sidebar and menu bar also offer **恢复所有屏幕**. This removes software dimming/blackout and requests reconnection of outputs stopped by ScreenPilot. If a monitor has been powered off or stops accepting DDC commands, use its power or input button to bring the picture back.
...
If Windows is still sending a signal, the monitor may continue showing Windows after Mac output stops. Reconnecting restores the saved screen arrangement where possible; some application windows may need repositioning. If restoration fails, use the retained recovery record to retry.

## Mac ↔ Windows setup

Connect **Mac to HDMI 1 and Windows to HDMI 2**. Keep the video cables connected to both computers; ScreenPilot selects which input the monitor displays.
...
4. Register your shared monitor using [hardware setup](docs/hardware-setup.md), then rebuild the Mac app with that configuration. This enables the shared-host buttons and USB host switching for your monitor. The download starts with an empty mapping; brightness, blackout and output controls are available separately.
...
Manual switching offers **保持 Mac 布局** or **移出 Mac 桌面**. The latter requires reconnection before switching back and may behave differently across monitors. Use the Windows local buttons for manual switching without pairing. USB automation needs the paired LAN connection.

## Compatibility and troubleshooting

- **Mac:** Apple Silicon only for the published binary; macOS 14+ deployment target. After a major macOS update, check the latest release for display-control compatibility fixes.
- **External displays:** DDC support depends on the monitor, active input, cable, dock and adapter. Hardware controls appear when the display can be identified and the relevant capability is available. If a control is unavailable, use software dimming or blackout; power commands are disabled for models with known problems.
- **Blank/noisy screen:** stop automation, try restore-all, then use physical input selection or reconnect the display cable if necessary. After sending an input command, check that the intended computer’s picture appears.
- **No shared-host buttons:** add your tested monitor configuration and rebuild the Mac app using [hardware setup](docs/hardware-setup.md).
...
Before enabling automatic switching, try manual switching in both directions and a timed output-stop/recovery cycle with your own monitor and cables.
...
See [contributors](CONTRIBUTORS.md). Original code and artwork: **all rights reserved**; see [LICENSE](LICENSE). Device discovery incorporates [m1ddc](https://github.com/waydabber/m1ddc) at commit `04d949794102eb8df01ad3681afff6464a3eede2`, under its [MIT license](Vendor/m1ddc/LICENSE).
...
屏幕管家把这些日常操作放在一起：想暂时不看某块屏幕、保留窗口布局，就用**软件黑屏**；想停用闲置桌面，让兼容显示器在无信号后进入待机，就用**停止 Mac 输出**。需要待机时，请在显示器菜单中开启无信号休眠；如果另一台电脑仍在输出画面，屏幕会继续显示该输入。
...
- **把整张桌面的设备协调起来。** 逐屏亮度、Mac 活动输出和配置后的 Windows 配套程序组成连续操作流程。配套程序可以从 Windows 一侧请求切源，适用于部分显示器不再接受非当前 Mac 输入命令的情况；首次设置时开启显示器的 DDC/CI，并分别测试两个方向的切换。
- **分清“画面黑了”和“输出停了”。** 软件黑屏保留桌面，停止输出则可能让闲置 LCD 进入待机。短暂休息时保留窗口位置，不再需要外屏时收起对应桌面，根据当前任务选择即可。
- **常用组合少点几次，恢复路径随时可见。** 命名预设、仅用内置屏、定时恢复和独立恢复程序，减少重复设置。操作会保留至少一块 Mac 活动屏幕；恢复失败时会提示原因，并保留重试入口。

支持的设备可使用**原生/DDC 硬件亮度**，不支持时提供明确标注的**软件调暗**。无需账户，无分析遥测；配对与开机启动由你主动设置。

**搭建双主机桌面：** 将 Mac、Windows 都连接到显示器，通过实体 USB 切换器共用键鼠，并让两台电脑接入局域网。按[硬件配置说明](docs/hardware-setup.md)登记已测试的显示器、构建带配置的 Mac 应用，再完成 Windows 配对。
...
Mac 版本为 **ad-hoc 签名，未使用 Developer ID 签名、未经过 Apple 公证**；Windows 程序**未使用 Authenticode 签名**。系统阻止启动时，先确认下载来源和校验值；macOS 可在提供该选项时使用**系统设置 → 隐私与安全性 → 仍要打开**。受管理电脑若阻止启动，请联系管理员。
...
侧栏和菜单栏均有**恢复所有屏幕**，用于清除软件调暗/黑屏，并请求恢复本应用停止的输出。如果显示器已断电或不再接受 DDC 命令，请用实体电源键或输入选择键恢复画面。
...
如果 Windows 仍在输出，停止 Mac 输出后，显示器可能继续显示 Windows。重新连接时会尽可能恢复保存的屏幕排列，部分应用窗口可能需要手动调整位置；恢复失败时可使用保留的记录重试。

### Mac ↔ Windows 配置

将 **Mac 接 HDMI 1、Windows 接 HDMI 2**。两台电脑的视频线保持连接，由屏幕管家选择显示器当前显示哪一路输入。
...
4. 按[硬件配置说明](docs/hardware-setup.md)登记已测试的共享显示器，并重新构建 Mac 应用，即可为该屏幕启用主机切换按钮和 USB 联动。下载包的初始映射为空；亮度、黑屏和输出控制可单独使用。
...
手动切换可选择**保持 Mac 布局**或**移出 Mac 桌面**，后者切回时需要重连，兼容性取决于设备。未配对时可使用 Windows 本地按钮手动切源；USB 自动切换需要已配对的局域网连接。

### 兼容性与排查

- **Mac：** 发布二进制仅支持 Apple Silicon，部署目标 macOS 14+；升级 macOS 大版本后，可查看最新发布中的显示器兼容性修复。
- **外屏：**DDC 能力与显示器、当前输入、线缆、扩展坞和转接器有关。设备身份与能力确认后会显示相应硬件控件；控件不可用时可使用软件调暗或黑屏。有已知电源问题的型号会停用电源命令。
- **黑屏/雪花：** 先停止自动切换、尝试恢复全部；必要时使用实体输入选择或重插显示器线缆。发送切源命令后，确认屏幕已显示目标电脑的画面。
- **没有主机切换按钮：** 按[硬件配置](docs/hardware-setup.md)添加已测试的显示器配置，并重新构建 Mac 应用。
...
开启自动切换前，请使用自己的显示器和线缆，先试一次双向手动切源，以及带倒计时的停止输出与恢复。
...
作者与维护者：**[QiushanHuang · Qiushan](https://github.com/QiushanHuang)**，详见[贡献者](CONTRIBUTORS.md)。项目原创代码和图形**保留所有权利**，见 [LICENSE](LICENSE)。设备发现使用 [m1ddc](https://github.com/waydabber/m1ddc) 固定提交 `04d949794102eb8df01ad3681afff6464a3eede2`，保留其作者署名及 [MIT 许可](Vendor/m1ddc/LICENSE)。
